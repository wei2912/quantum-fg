...
The \emph{identity operator} on any vector space $V$, $I_V$ (written as $I$ if
there is no chance of confusion), is defined by the equation $I_V\ket{v} \equiv
\ket{v}$ for all vectors $\ket{v}$.

The \emph{zero operator}, denoted by 0, maps all vectors to the zero vector,
$0\ket{v} \equiv 0$.

\paragraph{Matrix representation of a linear operator.} Suppose $A: V
\rightarrow W$ is a linear operator between vector spaces $V$ and $W$, and
...
[TODO: include part on change-of-basis matrix]
...
\begin{proof}
  ...
  \end{align*} as required. Equality occurs if and only if $\ket{v}$ and
  $\ket{w}$ are linearly related, $\ket{v} = z\ket{w}$ or $\ket{w} = z\ket{v}$,
  for some scalar $z$.
\end{proof}
...
is said to be \emph{diagonalizable} if it has a diagonal representation.
...
= A^\dagger A$. Clearly, an operator which is Hermitian is also normal.
